...
function nonprofit_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'nonprofit' ) . '</span></li>';
...
// Blog categories
function nonprofit_featured_post_cat(){
...
// social media
if ( ! function_exists( 'nonprofit_social' ) ) {
	function nonprofit_social( $args = array() ) {
		...
		$url = nonprofit_opt( 'nonprofit_social_url' );
...
// header cart count
function nonprofit_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( NONPROFIT_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'nonprofit' ); ?>">
...
function nonprofit_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'nonprofit_contact7_form_content', 10, 2 );