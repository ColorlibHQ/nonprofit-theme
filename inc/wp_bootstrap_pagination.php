...
function nonprofit_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'nonprofit' ),
        'next_string'     => esc_html__( '&raquo;', 'nonprofit' ),
        'before_output'   => '<div class="nonprofit-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'nonprofit_pagination_defaults', $defaults )
    );
